...
## 🛠️ SESSION UPDATE (2026-01-10)

### Critical Fix: EMA Indicator Computation for StrategySelectorAdapter

**Issue:** `EMA indicators missing or NaN: ema_5` warning appearing during scalping strategy execution.

**Root Cause:** The `_compute_ema_columns()` and `ensure_runtime_indicators()` functions in `core/trading_loop.py` were checking for `isinstance(strategy, StrategySelector)` but the actual strategy object is a `StrategySelectorAdapter` that wraps the `StrategySelector`. This meant the EMA period detection logic was bypassing the strategy inspection entirely.

**Fix Applied:** Updated all `isinstance(strategy, StrategySelector)` checks to also handle `StrategySelectorAdapter`:
```python
...
**Files Modified:**
- `core/trading_loop.py` - 5 locations updated

**Tests:** All 182 unit tests pass.

### Configuration Changes

1. **Hektor Disabled:** Set `hektor.enabled: false` in config.json
   - System will use SQLite fallback for vector operations
   - GGUF support available but not needed until Hektor phase
