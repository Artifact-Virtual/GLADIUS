...
class FallbackLLMProvider(LLMProvider):
    ...
    def __init__(self, config: "Config", logger: logging.Logger):
        ...
        elif getattr(config, 'PREFER_OLLAMA', False):
            self._init_ollama_first(config, logger)
        else:
    ...
    def _init_ollama_first(self, config, logger):
        """OLLAMA-first: Ollama → Gemini → Local"""
        # 1. Ollama primary
        self._try_init_ollama(config, logger, as_primary=True)
...
class Config:
    ...
    # PREFER_OLLAMA: Set to 1/true to prefer Ollama as primary provider (Ollama -> Gemini -> Local)
    PREFER_OLLAMA: bool = field(default_factory=lambda: os.environ.get("PREFER_OLLAMA", "").lower() in ("1", "true", "yes"))
