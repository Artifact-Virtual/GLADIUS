...
INGEST_ROOT = Path(__file__).parent / 'data' / 'ingest'
...
@app.route('/api/ingest/<name>/records')
def get_ingest_records(name: str):
    p = INGEST_ROOT / name / 'records.json'
    if not p.exists():
        return jsonify({'error': 'not found'}), 404
    return jsonify(json.loads(p.read_text(encoding='utf-8')))


@app.route('/api/ingest/<name>/run', methods=['POST'])
def run_ingest(name: str):
    body = request.get_json() or {}
    adapter = body.get('adapter')
    days = str(body.get('days', 1))

    script = Path(__file__).parent / 'deploy' / 'run_ingest.sh'
    if not script.exists():
        return jsonify({'error': 'run script not found'}), 500
    ...
    try:
        proc = __import__('subprocess').Popen(cmd, cwd=str(Path(__file__).parent), stdout=__import__('subprocess').DEVNULL, stderr=__import__('subprocess').DEVNULL)
...
@app.route('/api/arty/dbs')
def list_arty_dbs():
    """Return discovered DB files from the ARTY store (if configured)."""
    store = Path(__file__).parent / '..' / 'store'
    # prefer explicit env var
    env_store = os.environ.get('ARTY_STORE_DIR')
    if env_store:
        store = Path(env_store)
    from ingest_bot.pipeline.arty_integration import scan_arty_store
    manifest = scan_arty_store(store)
